<?php
	...
	if(isset($_SESSION['UserLoginID']))
	{
		...
		if(mysqli_num_rows($result) > 0)
		{
			...
			$to = $txt_email;
			$subject = "Post like.";
			$message = "Dear " .$txt_name . ", Someone like your the post. Thank you";
			$header = "From: Blog Up Site <zeerak.ali96@gmail.com>";
		
			if(mail($to,$subject,$message,$header))
			{
				$query = "UPDATE likes SET like_dislike = 1 WHERE UID = '$UserID' AND PID = '$PID'";
		...
		else
		{
			...
			$to = $txt_email;
			$subject = "Like Notif.";
			$message = "Dear " .$txt_name . ",  Your post has been liked. Thank you";
			$header = "From: Blog Up Site <zeerak.ali96@gmail.com>";
		
			if(mail($to,$subject,$message,$header))
			{
			
				$query = "INSERT INTO likes(PID, UID, like_dislike) VALUES('$PID', '$UserID', 1)";
			
				$new_like->insert($query);
				
			
				$totalLikes = $connect->query("SELECT count(like_dislike) as postlike FROM likes WHERE PID = '$PID' AND like_dislike = 1");$totaldisLikes = $connect->query("SELECT count(like_dislike) as postdislike FROM likes WHERE PID = '$PID' AND like_dislike = 0");
